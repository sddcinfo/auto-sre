...
Used from any repo whose CLI wires this scanner up (e.g. via a
``precommit`` subcommand)::

    <your-cli> precommit                 # scan current repo working tree
    <your-cli> precommit --repo ~/path/to/repo
    <your-cli> precommit --include-staged   # include already-staged files
    <your-cli> precommit --verbose       # show every hit, not just file names
...
def _rules() -> list[Rule]:
    return [
        ...
        # Most of these are defensive: meetings/ + test-fixtures/
        # are typically gitignored, but a developer renaming a
        # fixture or copying a real meeting into a new path would
        # trip these.
        Rule(
